...
const allowedProps = ["hostBaseUrl", "localBaseUrl"];

export function addServerSideRendering(app, handlebarsEngine) {
  const devEnv = app.get("env") === "development";

  const viewsDir = app.get("views").replace("src", process.env.DIST_DIR);
  ...
  const ssrEngine = function (viewFile, options, callback) {
    // Use files from Parcel build
    viewFile = viewFile.replace("src", process.env.DIST_DIR);
    ...
    const cssFilePath = relativeViewFilePath.replace(".js", ".css");
    ...
    for (const key of Object.keys(props._locals)) {
      if (props[key] === props._locals[key] && !_.includes(allowedProps, key)) {
        delete props[key];
      }
    }
    ...
    try {
      ...
      const viewOptions = {
        ...
        rootComponentSource: rootDir + relativeViewFilePath,
        cssFileSource: rootDir + cssFilePath,
